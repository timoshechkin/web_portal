...
<template>
  <div>
    <v-toolbar rounded class="elevation-0">
      <v-toolbar-title><h3>Входящие обращения</h3></v-toolbar-title>
      <v-spacer></v-spacer>
    ...
    >
      <v-card>
        ...
        <v-container style="color:white; background-color: #1E88E5;">
            ...
            <table style="border: none; border-spacing: 0;">
              <tbody>
                ...
                <tr>
                  ...
                  <td class="pl-5 pr-2"><div v-if="vis_grade">Оценка:</div></td><td>
                    <div v-if="vis_grade && status_app_fv=='Завершен'">                     
                      <div v-if="$store.getters.getUserId==user_ib_fv">
        ...
        <v-form>
          <v-container class="px-7 pt-7">
            <v-textarea
              ...
              clearable
              hide-details="true"
              label="Укажите причину низкой оценки!"
              v-model="comment_fv"
            ></v-textarea>
            <v-card-actions>
              <v-btn
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      applyComment(){
        if(this.comment_fv.length==0){
          this.snackText = 'Укажите причину!'
      ...
      setGrade(n, state){
        ...
        if(n==1 && state==1 && this.grade_fv==1){
          ...
          if(this.grade_fv!=this.grade_old_fv){
            if(this.grade_fv<3)this.addCommentDialog = true
            else this.sendGrade()
          }         
        }       
        // console.log('n: ',n)
      ...
      updateListMessage(id, process){
        var oXmlHttpRequest = this.getDataAppeal(id, process);
      ...
      closeFormViewAppeal(){
        ...
        this.user_ib_fv = ''
        this.vis_grade = false
      },
      getDataAppeal(id, process){
        var oXmlHttpRequest = new XMLHttpRequest();
        const strRequest = this.$store.getters.getUrlTracker + '?action=get_appeal_item&id=' + id + '&process=' + process;
        oXmlHttpRequest.open("get",strRequest, false);
        oXmlHttpRequest.send(null);
        return oXmlHttpRequest;
      },
      openFormViewAppeal(item){
        var oXmlHttpRequest = this.getDataAppeal(item.ID, item.process);
        var resJsonObj = JSON.parse(oXmlHttpRequest.responseText);
        ...
        if (oXmlHttpRequest.status == 200){
          ...
          for (index = 0; index < messagesPayload.length; ++index) {
            ...
            if(this.colors_users.length>0){
              if(this.colors_users.find(item => item.name_user == name_user) == undefined){
                this.colors_users.push({ 'name_user': name_user, 'color': this.colors[index_color] })
                index_color++;
              }
            }else{
              this.colors_users.push({ 'name_user': name_user, 'color': this.colors[index_color] })
          ...
          //console.log('colors_users: ', this.colors_users);
 
          this.messages_fv = resJsonObj.payload[0].messages;
          this.status_app_fv = resJsonObj.payload[1].default_value;
          this.service_name_fv = resJsonObj.payload[2].default_value.name;
          this.date_reg_app_fv = resJsonObj.payload[4].default_value==''?'':this.formatDateTime(resJsonObj.payload[4].default_value);
          this.date_close_app_fv = resJsonObj.payload[5].default_value==''?'':this.formatDateTime(resJsonObj.payload[5].default_value);
          this.reg_num_app_fv = resJsonObj.payload[6].default_value;
          this.comment_fv = ''
          this.stage_app_fv = typeof resJsonObj.payload[9].default_value=="undefined"?'':resJsonObj.payload[9].default_value.name;
          this.theme_app_fv = resJsonObj.payload[10].default_value;
          this.grade_fv = resJsonObj.payload[15].vote.mark
          this.vis_grade = resJsonObj.payload[15].vote.visible
          this.user_ib_fv = resJsonObj.payload[16].default_value 
          this.id_app_fv = item.ID;
          this.process_fv = item.process;
          this.setVisMessage();
          this.formViewAppeal = true;
        }else{
          this.snackText = 'Ошибка получения данных!';
          this.snackColor = 'red accent-2';
          this.snackbar = true;
        }
